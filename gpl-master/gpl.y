...
STABLE *wordTable; /* table of identifiers (experiment 1 added)*/
STABLE *curWordNode;
ITABLE *intTable; /* table of const int (experiment 1 added)*/
ITABLE *curIntNode;
CSTABLE *csTable; /* table of const str (experiment 1 added)*/
CSTABLE *curCSNode;
...
void tableOut()
{
	ITABLE *m=intTable->next;
	CSTABLE *n=csTable->next;
	STABLE *q=wordTable->next;
	printf("\n****************TABLES*****************\n");
	while(q!=NULL)
	{
...
int main(int argc,   char *argv[])
{
	...
	next_label=LAB_MIN;
	wordTable=(STABLE *)malloc(sizeof(STABLE));
	wordTable->next=NULL;
	wordTable->p=NULL;
	curWordNode=wordTable;
	intTable=(ITABLE *)malloc(sizeof(ITABLE));
	intTable->next=NULL;
	intTable->p=NULL;
	curIntNode=intTable;
...
STABLE *isIdentifier(char *p,STABLE *node)
{
	while(node!=NULL)
	{
		if(node->p)
		...
		return isIdentifier(p,node->next);
	}
	return NULL;
}

ITABLE *isConstInteger(int p,ITABLE *node)
{
	while(node!=NULL)
	{
		if(node->p)
		...
		return isConstInteger(p, node->next);
	}
	return NULL;
}

CSTABLE *isConstString(char *p,CSTABLE *node)
{
	while(node!=NULL)
	{
		if(node->p)
		...
		return isConstString(p,node->next);
	}
	return NULL;
}
...
int yylex()
{
	...
	if(letter())
	{
		...
		{
			STABLE *pos=isIdentifier(token,wordTable);
			if(pos==NULL)
			{
				lexeme=malloc(strlen(token+1));
				strcpy(lexeme,token);
				yylval.string=lexeme;
				pos=(STABLE *)malloc(sizeof(STABLE));
				pos->p=lexeme;
				pos->next=NULL;
				curWordNode->next=pos;
				curWordNode=pos;
			}
			else
			{
	...
	if(digit())
	{
		...
		retract();
		ITABLE* pos =isConstInteger(atoi(token),intTable);
		if(pos==NULL)
		{
			lexeme=malloc(strlen(token+1));
			strcpy(lexeme,token);
			...
			*i=atoi(lexeme);
			pos=(ITABLE *)malloc(sizeof(ITABLE));
			pos->p=i;
			pos->next=NULL;
			curIntNode->next=pos;
			curIntNode=pos;
	...
	if(character=='"')
	{
		...
		concat();
		CSTABLE *pos =isConstString(token,csTable);
		if(pos==NULL)
		{
			lexeme=malloc(strlen(token+1));
			strcpy(lexeme,token);
			yylval.string=lexeme;
			pos=(CSTABLE *)malloc(sizeof(CSTABLE));
			pos->p=lexeme;
			pos->next=NULL;
			curCSNode->next=pos;
			curCSNode=pos;
	...
	if(character=='!')
	{
		...
		else
		{
			...
			printf("(25,-)\n");
			return '!';
			//printf("error: ! is not expected\n"); //this line is added in experiment 1
			//exit(-1); //this line is added in experiment 1
			// this line is changed in experiment 1  return '!';
		}		
	}
